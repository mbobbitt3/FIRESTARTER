...
int asm_work_skl_xeonep_avx512_1t(threaddata_t* threaddata)
{
    if (*((unsigned long long*)threaddata->addrHigh) == 0) return EXIT_SUCCESS;
        ...
        "movabs $59, %%r10;" // reset-counter for L2-buffer with 222 cache lines accessed per loop (818.62 KB)
        "movabs $3003, %%r11;" // reset-counter for L3-buffer with 6 cache lines accessed per loop (1126.12 KB)
...
int asm_work_skl_xeonep_avx512_2t(threaddata_t* threaddata)
{
    if (*((unsigned long long*)threaddata->addrHigh) == 0) return EXIT_SUCCESS;
        ...
        "movabs $4505, %%r11;" // reset-counter for L3-buffer with 2 cache lines accessed per loop (563.12 KB)
